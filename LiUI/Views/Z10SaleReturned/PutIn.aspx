	...
	<script type="text/javascript">
	    $(function () {
	        ...
	        $("#xsubmit").click(function () {
	            ...
	            $.post("/Z10SaleReturned/SavePutIn", { 'orderStatus': orderStatus },
                function (data) {
                    if (data == 1) {
                        parent.$.jshowtip("入库处理完成！", "success");
	...
	<div id="contents">
		<div id="top">
			<div class="con clearfix">
				<div class="fl">
					<a href="javascript:;" class="btn img back" rel="/Z10SaleReturned/SaleReturnedList"><i class="icon i_back">
					</i>返回<b></b></a>
				</div>
			</div>
		</div>
		<div id="main">
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0" class="detail <% if (ViewData["IsDetail"]!=null){ Response.Write("readonly");}%>">
				...
				<tr>
					<td colspan="5">
						<table width="100%" id='oItem' class="list-table f12">
							<tr>
								<th>产品名称</th>
								<th class="w100 tc">应入库数量</th>
                                <th class="w100 tc">已入库数量</th>
								<th class="w100 tc">当前入库数量</th>
								<th class="w80">操作</th>
							</tr>
							...
							<tr>
                                ...
                                <td class="tr"><%=(System.Math.Abs(item.CountHappend ?? 0)).ToString("0.##") %></td>
				...
				<tr class="focus">
					...
					<td class="tc">
						<label class="lradio"><input type="radio" name="action" value="<%=(int)EAP.Logic.Z10.OrderStatus.InnedSome %>" /> 这个单据还没有结束，下次我还需要补充。</label>
						<label class="lradio"><input type="radio" name="action" value="<%=(int)EAP.Logic.Z10.OrderStatus.Inned %>" /> 入库已经完成但没有付款。</label>
