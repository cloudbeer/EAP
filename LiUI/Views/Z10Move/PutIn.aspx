	...
	<script type="text/javascript">
	    $(function () {
	        ...
	        $("#xsubmit").click(function () {
	            ...
	            $.post("/Z10Move/SavePutIn", { 'orderStatus': orderStatus },
                    function (data) {
                        if (data == 1) {
                            parent.$.jshowtip("调拨单入库处理完成！", "success");
		...
		<div id="main">
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0" class="detail <% if (ViewData["IsDetail"]!=null){ Response.Write("readonly");}%>">
				...
				<tr class="focus">
					...
					<td class="tc">
						<label class="lradio"><input type="radio" name="action" value="<%=(int)(EAP.Logic.Z10.OrderStatus.InnedSome) %>" /> 仅入库了一部分，下次我还需要补充。</label>
						<label class="lradio"><input type="radio" name="action" value="<%=(int)EAP.Logic.Z10.OrderStatus.Inned %>" /> 入库操作已完成。</label><br />
				...
				<tr class="action">
					<td></td>
					<td class="tc"><input type="button" value="确认入库" id="xsubmit" class="gbutton mr20" /></td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
