                ...
                $.post("/Z10Move/doCreate",
                    {
                        ...
                        if (data == 1) {
                            Li.isSubmit = true;
                            parent.$.jshowtip("调拨单已保存！", "success", 2);
                            location = "/Z10Move/MoveList";
                        } else {
                            parent.$.jshowtip("系统错误，请联系管理员！", "error", 2);
                        }
                    }
                );
            });
        ...
        <div id="main">
            ...
            <table cellspacing="0" cellpadding="0" width="100%" border="0" class="detail <% if (ViewData["IsDetail"]!=null){ Response.Write("readonly");}%>">
                ...
                <tr>
                    ...
                    <td class="tc">
                        <%
                            var orderItem = Model.Z10Order.GetTop1OrderItem(db);
                            var depot = orderItem.GetDepot(db); %>
                        <input type="hidden" id="depotout" value="<%=depot.DepotID %>" />
                        <%=depot.Title%>
                    </td>
                    <td class="tt">
                        入库仓库：
                    ...
                    <td class="tc">
                        <% var depot2 = orderItem.GetDepot2(db); %>
                        <input type="hidden" id="depotin" value="<%=depot2.DepotID %>" />
                        <%=depot2.Title%>
                    </td>
                    <td width="*">
                    </td>
                    ...
                    <td colspan="5">
                        <table width="100%" id='oItem' class="list-table f12">
                            ...
                            <% foreach (Z10Cabbage.Entity.Z10OrderItem item in Model.Items)
                               { %>
                            <tr class="readonly">
                                <td><%=item.GetProduct(db).Title%></td>
                                <td class="tr"><%=(item.GetV_DepotProduct(db).StockSum ?? 0).ToString("0.##")%></td>
                                <td class='tr' rel='item_countshould'><input class='text w100 tr' value="<%=(item.CountShould ?? 0).ToString("0.##")%>" /></td>
                                <td>
                                    <a href='javascript:;' class='i_xok xok none'>保存</a>
                ...
                <tr class="action">
                    ...
                    <td class="tc">
                        <input type="button" value="修改调拨单" id="xsubmit" class="gbutton mr20" />
                    </td>
                    <td>
                    </td>
                    <td>
