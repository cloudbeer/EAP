...
namespace Z01Beetle.Controllers
{
    ...
    public class Z01UnitController : EAP.Logic.__UserController
    {


        #region 验证
        protected void ValidateZ01Unit(Z01Unit entity)
        {
        ...
        #region 查询
        public ActionResult Index(int? PageIndex, int? PageSize, string qTitle, Int32? qUnitType, Int32? qUnitStatus, DateTime? qCreateDateStart, DateTime? qCreateDateEnd, int? orderCol)
        {
            ...
            hs.Add("qUnitType", qUnitType);
            hs.Add("qUnitStatus", qUnitStatus);
            hs.Add("qCreateDateStart", qCreateDateStart);
            hs.Add("qCreateDateEnd", qCreateDateEnd);

            PaginatedList<Z01Unit> result = Z01UnitHelper.Query(db, _tenant.TenantID.Value, currentPageSize, currentPageIndex, hs, orderCol);
        ...
        {
            if ((_crud & Zippy.SaaS.Entity.CRUD.Read) != Zippy.SaaS.Entity.CRUD.Read) return RedirectToAction("NoPermission", "Error");
            Z01Unit entity = Z01UnitHelper.Create(db, id);
            return View(entity);
        }
        #endregion
        ...
        {
            if ((_crud & Zippy.SaaS.Entity.CRUD.Delete) != Zippy.SaaS.Entity.CRUD.Delete) return Content("401");
            string sql = "UnitID in (" + ids + "0)";
            //return Content("1");

            if (db.Delete<Z01Unit>(sql) > 0)
                return this.Content("1");
            else
                return this.Content("0");
        }
        ...
        public ActionResult Edit(System.Int32? id, string act)
        {
            Z01Unit entity = null;
            if (id.HasValue && id > 0)
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [ValidateInput(false)]
        public ActionResult Edit(System.Int32? id, Z01Unit entity)
        {
            entity.UnitID = id;
            string formUnitType = Request.Form["UnitType"];
            entity.UnitType = formUnitType.ToEnumInt32();


            ValidateZ01Unit(entity);
